...
public class Quiz2Activity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        questionList = QuestionData.getQuestions4();
        tvOptionOne = findViewById(R.id.tv_option_one2);
        tvOptionTwo = findViewById(R.id.tv_option_two2);
        tvOptionThree = findViewById(R.id.tv_option_three2);
        btnSubmit = findViewById(R.id.btn_submit2);
        tvProgress = findViewById(R.id.tv_progress2);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tv_option_one2){
            selectedOptionView(tvOptionOne,1);
        }
        else if (v.getId() == R.id.tv_option_two2) {
            selectedOptionView(tvOptionTwo,2);
        }
        else if (v.getId() == R.id.tv_option_three2) {
            selectedOptionView(tvOptionThree,3);
        }
        else if (v.getId() == R.id.btn_submit2){
            if (selectedOption == 0){
                currentPosition++;
                if (currentPosition <= questionList.size()) {
                    setQuestion();
                } else {
                    Intent intent = new Intent(Quiz2Activity.this, ResultActivity.class);
    ...
    private void selectedOptionView(TextView tv, int selecteOptionNum) {
        ...
        tv.setBackground(ContextCompat.getDrawable(Quiz2Activity.this,R.drawable.selected_option));
    ...
    private void defaultOptionView() {
        ...
        for (int i = 0;i < options.size();i++){
            ...
            option.setBackground(ContextCompat.getDrawable(Quiz2Activity.this,R.drawable.default_option_border_bg));
        }
    }

    private void answersView(int answers, int drawableView) {
        if (answers == 1) {
            tvOptionOne.setBackground(ContextCompat.getDrawable(Quiz2Activity.this,drawableView));
        } else if (answers == 2) {
            tvOptionTwo.setBackground(ContextCompat.getDrawable(Quiz2Activity.this,drawableView));
        } else if (answers == 3) {
            tvOptionThree.setBackground(ContextCompat.getDrawable(Quiz2Activity.this,drawableView));
        }
    }
}